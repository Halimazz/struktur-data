...
void search(Tree **current, int huruf){
    cin >> huruf;
	//jika pointer current memiliki data
	if((*current)!=NULL){
		//cek, apakah datanya lebih kecil. Jika iya, belok ke kiri
		if(huruf<(*current)->huruf){
			search(&(*current)->left,huruf);
		//jika lebih besar, maka belok ke kanan
		}else if(huruf>(*current)->huruf){
			search(&(*current)->right,huruf);
		//jika sama dengan, maka angka ketemu
		}else{
			printf("Found : %d", (*current)->huruf);
		}
